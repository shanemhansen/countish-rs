...
pub fn main() {
    ...
    opts.optopt("", "support", "", "Base granularity. Eg. if your support is .1 you can't find content that occurs at frequency .01");
    opts.optopt("", "error-tolerance", "", "Tolerable error (eg .01 for 1%). Impls:stucky, lossy");
    opts.optopt("", "failure-prob", "", "Chances that incorrect results will be published. Impls:sticky");
    opts.optopt("", "threshold", "", "frequency threshold: return entries who's frequency exceeds this.");
    ...
    let (mut support, mut error_tolerance, mut failure_prob, mut threshold) = (0.0001, 0.0001,0.0001,0.05);
    ...
    let entries = match implementation.as_ref() {
        ...
        "sticky" => process(new_sampler(support, error_tolerance, failure_prob), threshold),
