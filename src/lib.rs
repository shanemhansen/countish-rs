...
impl NaiveSampler {
    ...
    pub fn items_above_threshold(&self,threshold: f64) -> Vec<Entry> {
        ...
        self.vals.iter()
            ...
            .map(|(key, valref)|
                 Entry{key:key.clone(), frequency:*valref as f64/self.n as f64})
            .collect()
    }
}
...
impl LossyCounter {
    fn prune(&mut self, bucket: u64) {
        ...
        let to_remove : Vec<String>= self.d.iter().filter(|&(_, value)|
                                            value.f+value.delta <= fbucket)
    ...
    pub fn items_above_threshold(&self, threshold :f64) -> Vec<Entry> {
        ...
        self.d.iter().filter(|&(_, val)|
                             val.f >= (threshold-val.delta)*f_n)
            .map(|(key, val)| Entry{key:key.clone(), frequency: val.f/f_n+self.support})
            .collect()
    ...
    pub fn observe(&mut self, key : &str) {
        ...
        let newval = match self.d.get(key) {
            ...
            _ => FDeltaPair{f:1.0, delta: (bucket - 1) as f64}
        };
...
impl StickySampler {
    ...
    pub fn items_above_threshold(&self, threshold: f64) -> Vec<Entry> {
        ...
        self.s.iter().filter(|&(_, f)|
                             *f >= (threshold-self.error_tolerance)*self.n)
            .map(|(key, f)|
                 Entry{key:key.clone(), frequency: *f/self.n+self.support})
            .collect()
    }
    pub fn observe(&mut self, key : &str) {
        self.n+=1.0;
